...
-  **说话添加日程**："明天下午三点开会" → 自动创建事件
-  **说话查看日程**："看看明天有什么安排" → 朗读日程摘要
-  **说话删除日程**："把开会删掉" → 确认后删除
-  **说话修改日程**："把开会改到四点" → 确认后修改
...
| **录音/播放** | 自写原生 AudioModule | - | 语音录制和 TTS 播放 |
...
| **意图解析** | DeepSeek API | v4-flash | 自然语言理解 |
...
双击 `start.bat` 或运行 `start.ps1`，自动启动后端 + Metro + 安装 App。
...
VocaCal 定位为**中文世界最好用的语音日历 App**。当前中文市场缺乏一个纯语音优先的独立日历应用 -- Google Calendar/Fantastical 主攻英文，小爱同学/天猫精灵依赖硬件生态。

**核心差异化：**

- 有温度的 AI 回复（不是冷冰冰的已添加，而是友好的自然语言反馈）
- 秒级语音响应（讯飞 ASR + DeepSeek NLU + 讯飞 TTS 全管线优化）
- 本地 SQLite 存储保护隐私，支持离线浏览
- 完整的增删改查语音交互
...
| 产品 | 核心壁垒 | VocaCal 可借鉴 |
|------|---------|---------------|
| Google Calendar + Assistant | 生态护城河 | 多模态状态动画体系 |
| Apple Siri + Calendar | 设备无缝衔接 | 简短结构化的语音反馈 |
| 小爱同学 | IoT 联动 | 中文口语化时间表达深度理解 |
| 滴答清单 TickTick | 任务+日程合一 | 多事件拆分 + 录音手势设计 |
| Fantastical | 极致 UI | 自然语言解析的实时视觉反馈 |
| Reclaim AI / Motion | AI 自动排期 | 习惯/日常任务概念 |

> 完整竞品分析、现状诊断、优化路线图详见 [docs/competitive-analysis.md](docs/competitive-analysis.md)
