...
注意：测试或正式使用前，请去对应产品页面获取免费额度或下单购买正式套餐（语音合成创建应用后默认有每日500次调用）；同时需要去控制台服务页面，获取API有效密钥（AppID、APIKey、APISecret）后再调用。
语音合成产品页面
语音合成服务页
...
vcn	string	是	发音人，可选值：请到控制台添加试用或购买发音人，添加后即显示发音人参数值	"x4_xiaoyan"
...
10010	instance not enough	引擎授权不足	请到控制台提交工单联系技术人员
10019	service read buffer timeout, session timeout	session超时	检查是否数据发送完毕但未关闭连接
10101	engine inavtive	引擎会话已结束	检查客户端是否会话已结束(收到status=2的响应)但还在发送数据。
10109	aiges error invalid data	请求文本长度非法	检查是否文本长度超出了限制
10110	service license not enough	引擎授权不足	请到控制台提交工单联系技术人员
10043	Syscall AudioCodingDecode error	音频解码失败	确保所传音频编码格式与请求参数保持一致，
10221	no useful connecton	服务端没有可用连接	请到控制台提交工单联系技术人员
...
11200	auth no license	功能未授权或授权到期	联系运营人员
11201	auth no enough license	业务会话总量超限或日流控超限	联系运营人员
11202	qps overflow	秒级流控超限	联系运营人员
11203	concurrency overflow	并发流控超限	联系运营人员
11204	group authentication failed	组鉴权未通过，即组内所有功能均受限	联系运营人员
10160	parse request json error	请求数据格式非法	检查请求数据是否是合法的json
10161	parse base64 string error	base64解码失败	检查发送的数据是否使用base64编码了
10163	param validate error:...	参数校验失败	根据错误信息描述传入正确参数与值
10200	read data timeout	读取数据超时	检查是否累计10s未发送数据并且未关闭连接
10313	invalid appid	appid和apikey不匹配或者没有传appid	检查appid是否合法
11503	server error :atmos return an error data	服务内部响应数据错误	请到控制台提交工单联系技术人员
11502	server error: too many datas in resp	服务配置错误	请到控制台提交工单联系技术人员
10118	server cannot parse response data	服务端无法解析后端响应数据	请到控制台提交工单联系技术人员
12001	-	用户没有正常发送完数据	-
99999	-	用户异常断开连接，或者会话超时断开连接	属于调用方客户端会话连接异常问题，请自查客户端
#调用示例
