方言大模型 协议
#一、 方言大模型描述
方言大模型，支持普通话，简单英语和202种方言全免切，无需显示指定语种。技术咨询可直接提交工单

#二、接口Demo
部分开发语言Demo如下，其他开发语言请参照文档进行开发，欢迎大家到讯飞开放平台社区 交流集成经验。

大模型方言语音识别 Demo java语言
大模型方言语音识别 Demo python语言 spark_slm_iat.py
...
支持202种方言免切换，具体可查看支持方言明细	是	mulacc
...
1:文本格式输出，不带属性,
2:json格式输出，带文字属性"wp":"n"和标点符号属性"wp":"p"	否	
dhw	会话热词，支持utf-8和gb2312；
取值样例：“dhw=gb2312;你好|大家”（对应gb2312编码）；
“dhw=utf-8;你好|大家”（对应utf-8编码）	string	最小长度:0, 最大长度:1024	否	
...
答：方言大模型除支持中英文外，还支持202种方言免切换，具体支持明细可点击下载查看-方言大模型支持方言明细 。
